...
function init()
{
	var platform = new H.service.Platform({
	  'app_id': 'U7LcUfRkfGtii17kefoa',
	  'app_code': 'H8SuelU_4b091EM3JTdjDQ'
	});
	...
	*/
	...
	map = new H.Map(
	  ...
	  {
		zoom: 12,
		 center: { lng: 151.20732, lat: -33.86785}
	  }
	);
...
function handleStationData(data)
{
	...
	for (var i in items)
	{
		...
		/* console.log(tokens);	*/
		stations[tokens[0]] = { nlc: Number(tokens[6]), name: tokens[0], lat: Number(tokens[4]), lon: Number(tokens[5]), lines: {} };
	}
	initialiseStations();
	requestLineData();
}
